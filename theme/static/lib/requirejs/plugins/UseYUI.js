...
(function () {
    ...
    return {
        load: function (name, req, load, config) {
            ...
            /* option to overwrite YUI instance stored in a closure in order to apply new YUI configuration (for WireIt lib) */
            var temp = name;
            var index = temp.indexOf("!");
            if (index !== -1) {
                var recreate = temp.substring(index + 1) === "recreate";
